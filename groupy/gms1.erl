...
init(Id, Grp, Master) ->
  ...
  % all slaves have the same list of peers and they elect the first node in the list as the leader.
  receive
...
leader(Id, Master, Slaves, Group) ->
  ...
  % A message {msg, Msg} is multicasted to all peers and
  % a message Msg is sent to the application layer
    {mcast, Msg} ->
      bcast(Id, {msg, Msg}, Slaves),
      Master ! Msg,
      leader(Id, Master, Slaves, Group);
  % a message, from a peer or the master, that is a request from a node to join the group.
  % The message contains both the process identifier of the application layer, Wrk, and the process
...
slave(Id, Master, Leader, Slaves, Group) ->
  ...
  % A message Msg is sent to the master
    {msg, Msg} ->
      Master ! Msg,
      slave(Id, Master, Leader, Slaves, Group);
  % a multicasted view from the leader.
  % A view is delivered to the master process.
