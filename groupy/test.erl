...
% These are messages that we can send to one of the workers.
% It will multicast it to all workers.
% They should (if everything works) receive the message at the same (logical) time.
freeze(Wrk) ->
  Wrk ! {send, freeze}.
